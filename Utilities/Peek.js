    ...
    $.get(url, async function (string) {
      ...
      for (let i = 0; i < hashArray.length; i++) {
        ...
        if (hash[0] === suffix.toUpperCase()) {
          ...
          const dictionary = await testDictionary(newVal);
      ...
      if (numBreaches == null) {
        ...
        const resultText1 = `This password has been leaked <h6 class = "part">${numBreaches}</h6> times.`;
        const resultText2 = `That is approximately <h6 class = "part">${numBreaches}%</h6> less than the <a style = "text-decoration: underline; font-size: 16px; color: white" class="navbar-brand" href="Utilities/About.html#jumpTo">most leaked password.</a
          >`;
        const resultText3 = `Assuming set length, it'd take up to <h6 class = "part">${numBreaches}</h6> to hack via <a style = "text-decoration: underline; font-size: 16px; color: white" class="navbar-brand" class="navbar-brand" href="Utilities/About.html#jumpTo">brute-force.</a
          >`;
